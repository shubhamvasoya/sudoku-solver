...
def recognize_sudoku_and_solve(image, model, old_sudoku):
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            ...
            try:
                ...
                # If resize fails likely due to empty image
                continue
    ...
    if (not old_sudoku is None) and two_matrices_are_equal(old_sudoku, grid, 9, 9):
        if(solver.all_board_non_zero(grid)):
            orginal_warp = write_solution_on_image(orginal_warp, old_sudoku, user_grid)
    # If this is a different board
    else:
        solver.solve_sudoku(grid) # Solve it
        if(solver.all_board_non_zero(grid)): # If we got a solution
...
def write_solution_on_image(image, grid, user_grid):
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            ...
            image = cv2.putText(image, text, (int(bottom_left_corner_x), int(bottom_left_corner_y)), 
